...
teamsDb.iterate(
  (value: { name: string; abbvr: string }, key) => {
    const team = {
      key: key,
      name: value.name,
      abbvr: value.abbvr,
...
function renderTeamsTable(teams: any[]) {
  const teamsTbody = document.getElementById(
    "teams-table-tbody"
  ) as HTMLTableElement;
  ...
  for (let index = 0; index < teams.length; index++) {
    ...
    teamPlayers.textContent = team.players.toString();
...
function renderTeamsSelectOptions(teams: any[]) {
  const newPlayerTeamSelect = document.getElementById(
    "player-team-id"
  ) as HTMLSelectElement;
